...
impl<'a> SsoPacker<'a> {
    ...
    pub fn build_protocol_12(&self, sso: &SsoPacket, sec_info: Option<&SsoSecureInfo>) -> BinaryPacket {
        ...
        head.write_bytes_with_prefix(&self.keystore.sigs.a2, Prefix::INT32 | Prefix::WITH_PREFIX)
            ; // tgt
        head.write_str(&sso.command, Prefix::INT32 | Prefix::WITH_PREFIX)
            ; // command
        head.write_str("", Prefix::INT32 | Prefix::WITH_PREFIX)
            ; // message_cookies (empty)
        head.write_str(&self.guid_hex(), Prefix::INT32 | Prefix::WITH_PREFIX)
            ; // guid
        head.write_str("", Prefix::INT32 | Prefix::WITH_PREFIX)
            ; // empty
        head.write_str(&self.app_info.current_version, Prefix::INT16 | Prefix::WITH_PREFIX)
            ;
        self.write_sso_reserved_field(&mut head, sec_info);

        let head_span = head.as_slice();
        let mut result =
            BinaryPacket::with_capacity(head_span.len() + sso.data.len() + 2 * 4);

        result
            .write_bytes_with_prefix(head_span, Prefix::INT32 | Prefix::WITH_PREFIX)
            ;
        result
    ...
    fn write_sso_reserved_field(&self, writer: &mut BinaryPacket, sec_info: Option<&SsoSecureInfo>) {
        ...
        writer.with_length_prefix::<u32, _, _>(true, 0, |w| {
            ...
            if self.protocol.is_android() {
                ...
                w.write((21u32 << 3) | 0); // field 21, wire type 0 (varint)
                w.write(32u32); // msg_type = 32

                // Field 26: NtCoreVersion (uint32)
                w.write((26u32 << 3) | 0); // field 26, wire type 0
