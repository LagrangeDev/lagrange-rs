...
mod tests {
    ...
    #[test]
    fn test_service_request_builder_pattern() {
        use crate::internal::services::message::{SendMessageEvent, SendMessageResponse};

        // Test request builder
        let request = SendMessageEvent::builder()
            .target(123456)
        ...
        assert_eq!(request.target, 123456);
        assert_eq!(request.content, "Hello, World!");
        assert!(request.is_group);

        // Test response builder
        let response = SendMessageResponse::builder()
            .message_id(789)
            .time(1234567890)
            .success(true)
            .build()
        ...
        assert_eq!(response.message_id, 789);
        assert_eq!(response.time, 1234567890);
        assert!(response.success);
    }

    #[test]
    fn test_service_request_constructor() {
        use crate::internal::services::message::SendMessageEvent;

        // Test that the original constructor still works
        let request = SendMessageEvent::new(123456, "Hello".to_string(), false);
    ...
    #[test]
    fn test_service_request_accessors() {
        use crate::internal::services::message::SendMessageEvent;

        let request = SendMessageEvent::new(999, "Test".to_string(), true);

        // Test field accessors
        assert_eq!(*request.target(), 999);
        assert_eq!(request.content(), "Test");
        assert_eq!(*request.is_group(), true);
    }
}
